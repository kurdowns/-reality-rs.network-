...
public class EnchantingMagic {

	public enum Enchant {

		SAPPHIRERING(1637, 2550, 7, 18, 719, 114, 1), SAPPHIREAMULET(1694, 1727, 7, 18, 719, 114,
				1), SAPPHIRENECKLACE(1656, 3853, 7, 18, 719, 114, 1),

		EMERALDRING(1639, 2552, 27, 37, 719, 114, 2), EMERALDAMULET(1696, 1729, 27, 37, 719, 114,
				2), EMERALDNECKLACE(1658, 5521, 27, 37, 719, 114, 2),

		RUBYRING(1641, 2568, 47, 59, 720, 115, 3), RUBYAMULET(1698, 1725, 47, 59, 720, 115, 3), RUBYNECKLACE(1660,
				11194, 47, 59, 720, 115, 3),

		DIAMONDRING(1643, 2570, 57, 67, 720, 115, 4), DIAMONDAMULET(1700, 2570, 57, 67, 720, 115,
				4), DIAMONDNECKLACE(1662, 11090, 57, 67, 720, 115, 4),

		DRAGONSTONERING(1645, 2572, 68, 78, 721, 116, 5), DRAGONSTONEAMULET(1702, 1712, 68, 78, 721, 116,
				5), DRAGONSTONENECKLACE(1664, 11105, 68, 78, 721, 116, 5),

		ONYXRING(6575, 6583, 87, 97, 721, 452, 6), ONYXAMULET(6581, 6585, 87, 97, 721, 452, 6), ONYXNECKLACE(6577,
				11128, 87, 97, 721, 452, 6);
	...
	public static void enchantItem(Player c, int itemID, int spellID) {
		...
		if (c.level[PlayerConstants.MAGIC] >= enc.getLevelReq()) {
			if (c.getItems().playerHasItem(enc.getUnenchanted(), 1)) {
				if (hasRunes(c, spellID)) {
					if (getEnchantmentLevel(spellID) == enc.getELevel()) {
						...
						c.getPA().addSkillXP(enc.getXp(), PlayerConstants.MAGIC);
						...
						c.gfx100(enc.getGFX());
						if (ens.getReq3() != -1) {
							c.getItems().deleteItem(ens.getReq3(), c.getItems().getItemSlot(ens.getReq3()),
									ens.getReqAmt3());
						}
