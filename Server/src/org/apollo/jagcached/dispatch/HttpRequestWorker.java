...
public final class HttpRequestWorker extends RequestWorker<HttpRequest, ResourceProvider> {
     ...
     * cached crc buffer
     */
    private static ChannelBuffer crcBuffer = null;
    ...
    protected void service(ResourceProvider provider, Channel channel, HttpRequest request) throws IOException {
        ...
        if (path.equalsIgnoreCase("/checksums")) {
            if (crcBuffer == null) {
                StringBuilder bldr = new StringBuilder();

                try {
                    File directory = new File("./data/www/files/");
                    for (File file : directory.listFiles()) {
                        try {
                            byte[] b = Files.readAllBytes(file.toPath());
                            byte[] hash = MessageDigest.getInstance("MD5").digest(b);

                            bldr.append(file.getName() + "::" + DatatypeConverter.printHexBinary(hash) + System.lineSeparator());
                ...
                crcBuffer = ChannelBuffers.copiedBuffer(bldr.toString(), Charset.defaultCharset());
            }
            wrappedBuf = crcBuffer;
        } else if (buf == null) {
