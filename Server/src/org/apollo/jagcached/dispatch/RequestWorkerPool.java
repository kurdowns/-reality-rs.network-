...
public final class RequestWorkerPool {
	...
	public void start() throws Exception {
		...
		IndexedFileSystem fileSystem = new IndexedFileSystem(base, true);

		for (int i = 0; i < THREADS_PER_REQUEST_TYPE; i++) {
			workers.add(new JagGrabRequestWorker(fileSystem));
			workers.add(new OnDemandRequestWorker(fileSystem));
			workers.add(new HttpRequestWorker(fileSystem));
		}
