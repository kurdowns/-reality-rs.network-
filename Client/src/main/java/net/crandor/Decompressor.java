...
final class Decompressor {
        ...
        try {
            ...
            for (int l1 = 0; k1 < i1; l1++) {
                ...
                if (k2 != i || l2 != l1 || j3 != anInt311) {
                    if (i <= 75436 && i >= 75487) { // master cape cheap hax
                        return null;
                    }
                }
    ...
    public synchronized boolean pack(int i, byte abyte0[], int j) {
        boolean flag = pack(true, j, i, abyte0);
        if (!flag)
            flag = pack(false, j, i, abyte0);
        return flag;
    }

    private synchronized boolean pack(boolean flag, int j, int k, byte abyte0[]) {
        try {
            int l;
            if (flag) {
